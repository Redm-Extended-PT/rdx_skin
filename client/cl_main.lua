...
AddEventHandler('rdx:onPlayerDeath', function(data)
    isDead = true
end)

AddEventHandler('rdx:onPlayerSpawn', function(spawn)
    isDead = false
	TriggerServerEvent("rdx_skin:loadSkin", function(cb)
        end)
	Citizen.Wait(1000)
end) 
...
RegisterNetEvent('rdx_skin:openCreator')
AddEventHandler('rdx_skin:openCreator', function()
    local ped = PlayerPedId()
    local pp = GetEntityCoords(ped)
    Wait(500)
    SetEntityCoords(ped, -328.95, 775.33, 120.53)
...
RegisterCommand("skin", function(source, args, rawCommand)
    TriggerEvent("rdx_skin:openCreator", function(cb)
        end)
end)
...
RegisterNUICallback('saveSkin', function(data, cb)
    ...
    DestroyCam(cam, true)
    TriggerServerEvent("rdx_skin:createSkin", data, function(cb)
        if cb then
            print("CREATED")
        else
            print("ERROR")
...
RegisterNetEvent('rdx_skin:applySkin')
AddEventHandler('rdx_skin:applySkin', function(_data, target , clothes)
    Citizen.CreateThread(function()
        local _target
        ...
        if _t == nil then
            ...
            Wait(500)
            TriggerEvent("rdx_clothing:load", data, _clothes, _target)
        end
    end)
end)

RegisterNUICallback('updateBody', function(data, cb)
    TriggerEvent("rdx_skin:updateBody" , data)
end)

RegisterNetEvent('rdx_skin:updateBody')
AddEventHandler('rdx_skin:updateBody', function(data)
